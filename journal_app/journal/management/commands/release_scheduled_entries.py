...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        entries = Entry.objects.filter(is_scheduled=True, released=False, scheduled_time__lte=timezone.now())
        for entry in entries:
            for contact in entry.contact.all():
                if contact.email and entry.user.email_verified:
                    subject = f"{entry.user} has shared a journal entry with you, read it here!"
                    contact_mail = Mail(
                        user=entry.user,
                        subject=subject,
                        to=contact.email,
